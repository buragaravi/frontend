...
const FacultyDashboard = () => {
  ...
  const buttonActive = isDark
    ? 'bg-gradient-to-r from-[#0B3861] via-[#1E88E5] to-[#0B3861] text-white shadow-lg border border-[#BCE0FD] scale-105'
    : 'bg-gradient-to-r from-[#F5F9FD] via-white to-[#F5F9FD] text-[#0B3861] shadow-lg border border-[#BCE0FD] scale-105';
  ...
  return (
    ...
    >
      ...
      <button
        className="md:hidden fixed top-4 left-4 z-50 bg-[#F5F9FD] text-[#0B3861] p-2 rounded-full shadow-lg border border-[#BCE0FD]"
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 w-72 rounded-r-fullfull flex flex-col backdrop-blur-2xl shadow-2xl border transition-all duration-300 ${sidebarBg} ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        } md:translate-x-0 z-40`}
        style={{
          boxShadow: isDark
            ? '0 12px 48px 0 rgba(11, 56, 97, 0.28)'
            : '0 12px 48px 0 rgba(100, 181, 246, 0.18)',
        }}
      >
          ...
          <div className="p-6 border-t border-[#BCE0FD]/30">
            {loading ? (
              <div className="w-full flex justify-center items-center py-4">
                <span className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#64B5F6]"></span>
              </div>
            ) : user ? (
              <div className="w-full flex flex-col items-center gap-2">
                <div className="flex flex-col items-center">
                  <div className="w-14 h-14 rounded-full bg-[#BCE0FD] flex items-center justify-center text-2xl font-bold text-[#0B3861] shadow-md mb-1">
                    {user.avatar ? (
                      <img
                        src={user.avatar}
                        alt="avatar"
            ...
            <button
              ...
              className={`mt-4 w-full py-2 rounded-xl font-semibold transition-all duration-300 ${
                isDark
                  ? 'bg-[#0B3861] text-[#BCE0FD] border border-[#BCE0FD] hover:bg-[#64B5F6] hover:text-white'
                  : 'bg-[#F5F9FD] text-[#0B3861] border border-[#BCE0FD] hover:bg-[#64B5F6] hover:text-white'
              }`}
            >
              Logout
            </button>
