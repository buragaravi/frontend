...
const LabRequestListPage = ({ labId: propLabId }) => {
  ...
  const fetchLabRequests = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`https://pharmacy-stocks-backend.onrender.com/api/requests/lab/${labId}`, {
                    ...
                    <>
                      {req.status === 'pending' && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleOpenFulfill(req);
                          }}
                          className={`px-3 py-1 ${THEME.secondaryBg} text-white rounded-lg font-medium ${THEME.hoverBg} transition-colors text-xs`}
                        >
                          Fulfill/Reject
                        </button>
                      )}
